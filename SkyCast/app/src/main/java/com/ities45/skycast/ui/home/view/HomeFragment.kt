...
class HomeFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.onlineCurrentForecast.observe(requireActivity()) { current ->
            ...
            binding.tvDate.text = "Today, " + SimpleDateFormat("EEE MMM d", Locale.getDefault()).format(Date((current.dt * 1000).toLong()))
            ...
            binding.tvShortDate.text = SimpleDateFormat("EEE, MMM d", Locale.getDefault()).format(Date((current.dt * 1000).toLong()))
        }
    }
}